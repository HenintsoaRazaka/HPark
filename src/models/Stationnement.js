...
//             (proprietaire, telephone, cin, marque_modele, immatriculation, couleur, numero_parking, heure_entree, statut) 
//             VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), 'en_cours')
//         `;
...
//         const pool = dbSingleton.getPool();
//         const sql = `SELECT * FROM stationnement WHERE id = ?`;
...
//     static async updateSortie(id, heureSortie, prixTotal) {
//         const pool = dbSingleton.getPool();
//         const sql = `
//             UPDATE stationnement 
//             SET heure_sortie = ?, prix = ?, statut = 'termine' 
//             WHERE id = ?
//         `;
//         const [result] = await pool.execute(sql, [heureSortie, prixTotal, id]);
//         return result.affectedRows > 0;
//     }

//     static async checkPlaceOccupee(numeroParking) {
//         const pool = dbSingleton.getPool();
//         const sql = `
//             SELECT id FROM stationnement 
//             WHERE numero_parking = ? AND statut = 'en_cours'
//         `;
//         const [rows] = await pool.execute(sql, [numeroParking]);
//         return rows.length > 0;
//     }

//     static async findAll() {
//         const pool = dbSingleton.getPool();
//         const sql = `SELECT * FROM stationnement ORDER BY id DESC`;
//         const [rows] = await pool.execute(sql);
//         return rows;
//     }
// }
